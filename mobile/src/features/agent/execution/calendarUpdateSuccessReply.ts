...
export function buildNaturalCalendarUpdateSuccessReply(params: {
  ...
  if (locale === 'uk') {
    reply = `Подію оновлено успішно:\nНазва: ${exactTitle}\nНовий час: ${scheduleLabel}`;
  } else if (locale === 'ru') {
    reply = `Событие обновлено успешно:\nНазвание: ${exactTitle}\nНовое время: ${scheduleLabel}`;
  } else {
    reply = `Event updated successfully:\nTitle: ${exactTitle}\nNew time: ${scheduleLabel}`;
  }
