...
export function buildIntentPrioritySystemPrompt(analysis: AssistantIntentAnalysis): string | null {
  if (analysis.hasHardOperationalIntent || analysis.shouldBypassEmotionalRouting) {
    ...
    return [
      ...
      'OPERATIONAL EXECUTION: Never claim an action succeeded unless a tool verified success this turn. Forbidden without API success: "Записал", "Добавил", "Отправил", "Создал", "I added", "I scheduled", "Event added".',
      'Never use stall phrases: "one minute", "I\'ll do it now", "let me add it", "сейчас сделаю", "минуточку" — unless a NEW tool execution started this turn.',
      'If a calendar tool already returned FAILURE or PENDING, do NOT restart create — explain the last tool status only.',
      'Google Calendar create/update from chat is executed by tools, not by you. Never refuse with "cannot add right now", "prepare a draft only", or "when possible" for calendar writes — report only the last verified tool result.',
      'For SMS/call channels that are not wired, you may offer a draft — but never use that pattern for calendar events when Calendar is connected.',
